...
vec3 normalize(const vec3 &x) { return glm::normalize(x); }
...
vec3 Sphere::get_normal(const vec3& point) { return normalize(point - position); }
...
vec3 intersect_color(const vec3 origin, vec3 dir, float intensity, std::vector<Object*> &scene) {
    ...
    const vec3 PL = normalize(light_point - P);
    const vec3 PO = normalize(origin - P);
...
void rendering(int w, int h, std::vector<Object*> &scene, std::string filename) {
    ...
    for (int i = 0; i < w; ++i) {
        ...
        for (int j = 0; j < h; ++j) {
            ...
            vec3 color = intersect_color(O, normalize(Q - O), 1, scene);
